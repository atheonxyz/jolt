...
for (const p of PROGRAMS) {
    programState[p] = { loadState: 'idle', proofBytes: null, programIoBytes: null, cpuElapsed: null, gpuElapsed: null, pendingCompare: null };
}
...
function setButtonsEnabled(program, prove, verify) {
    ...
    page.querySelector('.prove-cpu-btn').disabled = !prove;
    page.querySelector('.prove-gpu-btn').disabled = !prove;
    page.querySelector('.compare-btn').disabled = !prove;
        ...
        worker.onmessage = (e) => {
            ...
            if (msg.type === 'init-done') {
                ...
                for (const p of PROGRAMS) {
                    ...
                    page.querySelector('.prove-cpu-btn').disabled = false;
                    page.querySelector('.prove-gpu-btn').disabled = false;
                    page.querySelector('.compare-btn').disabled = false;
            ...
            if (msg.type === 'prove-done') {
                ...
                if (msg.trace) st.traceJson = msg.trace;

                if (msg.mode === 'cpu') st.cpuElapsed = msg.elapsed;
                if (msg.mode === 'gpu') st.gpuElapsed = msg.elapsed;

                const modeLabel = msg.mode === 'gpu' ? 'GPU' : 'CPU';
                log(p, `Proof generated in ${(msg.elapsed / 1000).toFixed(2)}s (${modeLabel})`);
                ...
                if (st.pendingCompare) {
                    if (st.pendingCompare.modes.length > 0) {
                        const nextMode = st.pendingCompare.modes.shift();
                        setStatus(`Comparing (${nextMode.toUpperCase()})...`, 'proving');
                        worker.postMessage({
                            type: 'prove',
                            data: { ...st.pendingCompare.payload, mode: nextMode },
                        });
                    } else {
                        st.pendingCompare = null;
                        showComparison(p);
...
function setupProveHandlers() {
    function proveSha2(mode) {
        return async () => {
            ...
            setStatus(`Proving (${mode.toUpperCase()})...`, 'proving');
            log('sha2', `\nProving SHA-256("${message}") [${input.length} bytes] (${mode.toUpperCase()})`);
            worker.postMessage({
                type: 'prove',
                data: { program: 'sha2', mode, input: Array.from(input) },
            });
        };
    }

    function proveEcdsa(mode) {
        return async () => {
            setButtonsEnabled('ecdsa', false, false);
            if (!await ensureProgramLoaded('ecdsa')) { updateTabButtons('ecdsa'); return; }
            setStatus(`Proving (${mode.toUpperCase()})...`, 'proving');
            log('ecdsa', `\nProving ECDSA signature verification ("hello world") (${mode.toUpperCase()})`);
            worker.postMessage({
                type: 'prove',
                data: {
                    program: 'ecdsa',
    ...
    function proveKeccak(mode) {
        return async () => {
            ...
            setStatus(`Proving (${mode.toUpperCase()})...`, 'proving');
            log('keccak', `\nProving Keccak chain("${message}", ${numIters} iters) (${mode.toUpperCase()})`);
            worker.postMessage({
                type: 'prove',
                data: { program: 'keccak', mode, input: Array.from(input), numIters },
            });
        };
    }

    document.querySelector('#page-sha2 .prove-cpu-btn').addEventListener('click', proveSha2('cpu'));
    document.querySelector('#page-sha2 .prove-gpu-btn').addEventListener('click', proveSha2('gpu'));
    document.querySelector('#page-ecdsa .prove-cpu-btn').addEventListener('click', proveEcdsa('cpu'));
    document.querySelector('#page-ecdsa .prove-gpu-btn').addEventListener('click', proveEcdsa('gpu'));
    document.querySelector('#page-keccak .prove-cpu-btn').addEventListener('click', proveKeccak('cpu'));
    document.querySelector('#page-keccak .prove-gpu-btn').addEventListener('click', proveKeccak('gpu'));
}
...
function setupTraceHandlers() {
    for (const p of PROGRAMS) {
        document.querySelector(`#page-${p} .trace-btn`).addEventListener('click', () => {
            // Use cached inline trace if available (avoids WASM OOM on get_trace_json)
            const st = programState[p];
            if (st && st.traceJson) {
                const blob = new Blob([st.traceJson], { type: 'application/json' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `jolt-trace-${Date.now()}.json`;
...
async function buildProvePayload(program) {
    switch (program) {
        case 'sha2': {
            const message = document.getElementById('sha2-message').value;
            const input = new TextEncoder().encode(message);
            return { program: 'sha2', input: Array.from(input) };
        }
        case 'ecdsa':
            return {
                program: 'ecdsa',
        ...
        case 'keccak': {
            const message = document.getElementById('keccak-message').value;
            const numIters = parseInt(document.getElementById('keccak-iters').value, 10);
            if (numIters < 1 || numIters > 100) return null;
            const messageBytes = new TextEncoder().encode(message);
            const input = await sha256Digest(messageBytes);
            return { program: 'keccak', input: Array.from(input), numIters };
        }
    }
}

function showComparison(program) {
    const st = programState[program];
    const cpuSec = st.cpuElapsed / 1000;
    const gpuSec = st.gpuElapsed / 1000;
    log(program, '');
    log(program, '══════ Comparison ══════');
    log(program, `CPU: ${cpuSec.toFixed(2)}s`);
    log(program, `GPU: ${gpuSec.toFixed(2)}s`);
    if (gpuSec < cpuSec) {
        const speedup = cpuSec / gpuSec;
        log(program, `GPU is ${speedup.toFixed(2)}x faster`);
    } else if (cpuSec < gpuSec) {
        const speedup = gpuSec / cpuSec;
        log(program, `CPU is ${speedup.toFixed(2)}x faster`);
    } else {
        log(program, 'Both are equal');
    }
    log(program, '═══════════════════════');
...
function setupCompareHandlers() {
    for (const p of PROGRAMS) {
        document.querySelector(`#page-${p} .compare-btn`).addEventListener('click', async () => {
            const st = programState[p];
            const needed = [];
            if (st.cpuElapsed == null) needed.push('cpu');
            if (st.gpuElapsed == null) needed.push('gpu');
            ...
            setButtonsEnabled(p, false, false);
            if (!await ensureProgramLoaded(p)) { updateTabButtons(p); return; }

            const payload = await buildProvePayload(p);
            if (!payload) {
                log(p, 'Invalid input. Check parameters.');
                updateTabButtons(p);
                return;
            }

            st.pendingCompare = { modes: [...needed], payload };
            const firstMode = st.pendingCompare.modes.shift();
            setStatus(`Comparing (${firstMode.toUpperCase()})...`, 'proving');
            log(p, '\nComparing CPU vs GPU...');
            worker.postMessage({
                type: 'prove',
                data: { ...payload, mode: firstMode },
            });
        });
    }
}
