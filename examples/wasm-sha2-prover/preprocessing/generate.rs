...
struct ProgramSpec {
    ...
    preprocess_verifier: fn(
        JoltSharedPreprocessing,
        ArkworksVerifierSetup,
        Option<BlindfoldSetup<Bn254Curve>>,
    ) -> VerifierPrep,
}
...
fn generate_program(www_dir: &Path, spec: &ProgramSpec) {
    ...
    prover_preprocessing
        .serialize_uncompressed(&mut prover_bytes)
        .expect("Failed to serialize prover preprocessing");
    write_file(www_dir, spec.prover_file, name, "Prover", &prover_bytes);
