use guest::types::{GuestInputs, GuestPassportValidity};
use std::time::Instant;
use tracing::info;

#[rustfmt::skip]
fn load_inputs() -> GuestInputs {
    GuestInputs {
...
min_age_required : 18,
max_age_required : 70,
        passport_validity: GuestPassportValidity {
...
pub fn main() {
    ...
    info!("  DG1 padded length: {}", inputs.dg1_padded_length);
    info!(
        "  Age range: [{}, {}]",
        inputs.min_age_required, inputs.max_age_required
    );
    ...
    // ── Prove ──
    // Cycle tracking section counts appear in RUST_LOG=info output here,
    info!("Starting proof generation...");
    let now = Instant::now();
    let (output, proof, io_device) = prove(inputs.clone());
    ...
    let now = Instant::now();
    let is_valid = verify(inputs, output, io_device.panic, proof);
