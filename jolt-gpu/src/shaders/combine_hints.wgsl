...
fn read_g1j(words: ptr<storage, array<u32>, read>, base: u32) -> G1Jacobian {
    var out: G1Jacobian;
    for (var i = 0u; i < 8u; i = i + 1u) {
        out.x.limbs[i] = (*words)[base + i];
        out.y.limbs[i] = (*words)[base + 8u + i];
        out.z.limbs[i] = (*words)[base + 16u + i];
    }
    return out;
}

fn write_g1j(words: ptr<storage, array<u32>, read_write>, base: u32, pt: G1Jacobian) {
    for (var i = 0u; i < 8u; i = i + 1u) {
        (*words)[base + i] = pt.x.limbs[i];
        (*words)[base + 8u + i] = pt.y.limbs[i];
        (*words)[base + 16u + i] = pt.z.limbs[i];
    }
}
...
fn combine_hints_kernel(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    while (bit >= 0) {
        ...
        for (var i = 0u; i < ch_params.num_polys; i = i + 1u) {
            ...
            if (get_scalar_bit(scalar, u32(bit))) {
                ...
                let pt = read_g1j(&ch_points, point_base);
                acc = g1_add(acc, pt);
            }
        }
    ...
    write_g1j(&ch_results, row * 24u, acc);
}
