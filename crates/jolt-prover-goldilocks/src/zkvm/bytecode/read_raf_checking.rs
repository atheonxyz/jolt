//! Bytecode read + RAF checking sumcheck — ported from jolt-core's
//! `zkvm/bytecode/read_raf_checking.rs` onto [`crate::framework`] over the lean `Field`
//! (`C = F = Fp3`). jolt-core is the parity oracle.
//!
//! A single batched sumcheck over the `(address, cycle)` hypercube proving a γ-weighted RLC of
//! per-stage bytecode read claims (and RAF claims, which are just stages whose `Val_s = Int`, the
//! identity column):
//!
//! ```text
...
//! ```
//!
//! where the bytecode read indicator `ra(k,j) = ∏_{i=0}^{d-1} ra_i(k_i, j)` is the product of `d`
//! one-hot **address-chunk** polynomials (`k = (k_0,…,k_{d-1})`), `eq_s(j)` is the per-stage cycle
//! eq table, and `Val_s(k)` is the address-only per-stage value column (encoding circuit flags /
//! lookup-table membership / the identity for RAF). The chunk product makes the cycle-binding
//! rounds degree `d+1`; this port fixes **d = 2** (degree-3, the handoff's stated bytecode degree).
//!
//! Caches the `d` per-chunk `ra_i` leaf openings `ra_i(r_addr_chunk_i, r_cycle)` as
//! `CommittedPolynomial::BytecodeRa(i)` under [`SumcheckId::BytecodeReadRaf`]. **These leaf claims
//! are exactly the §4.5.2 inputs to the M7 LogUp\*-GKR pushforward** — the read-raf sumcheck itself
//! is unchanged by M7; only how these `ra_i` leaves are committed/opened changes (one-hot →
...
//! `ra_i` (broadcast to the full hypercube here for uniform single-phase binding) and the public
//! per-stage `(eq cycle point, Val address column)`. Deferred: jolt-core's two-phase
//! address-then-cycle materialization + Gruen split-eq, the entry-point constraint (`f_entry·C`),
//! the flag/lookup-table-specific `Val_s` construction, and the `d`-chunk one-hot *commitment*
//! (reworked by M7).
...
/// One batched stage: a per-stage cycle-eq point `r_cycle_s`, the public address-only value column
/// `val_addr_s` (length `K`), and the accumulator key of the upstream read claim `rv_s`.
#[derive(Clone, Debug)]
...
#[derive(Clone, Debug)]
pub struct BytecodeReadRafParams<F: Field> {
    /// `[γ^0, γ^1, …, γ^{S-1}]` for `S` stages.
    pub gamma_powers: Vec<F>,
    /// Address-chunk bit widths `[log_K_0, log_K_1]` (so `log_K = log_K_0 + log_K_1`).
    pub log_k_chunks: [usize; NUM_CHUNKS],
    pub log_t: usize,
    pub stages: Vec<ReadRafStage<F>>,
}

impl<F: Field> BytecodeReadRafParams<F> {
...
/// keeps the per-stage `r_cycle`/`val_addr` (public) to recompute `eq_s(ρ)`/`Val_s(ρ)`.
pub struct BytecodeReadRaf<F: Field> {
    pub params: BytecodeReadRafParams<F>,
    ra: [MultilinearPolynomial<F>; NUM_CHUNKS],
    /// Per-stage `eq_s(j)·Val_s(k)` is kept as the two factors broadcast to the full hypercube.
...
impl<F: Field> BytecodeReadRaf<F> {
    ...
    pub fn new_prover(params: BytecodeReadRafParams<F>, ra_chunks: [Vec<F>; NUM_CHUNKS]) -> Self {
        let t = 1usize << params.log_t;
        let k0 = 1usize << params.log_k_chunks[0];
        let k1 = 1usize << params.log_k_chunks[1];
        let n = k0 * k1 * t;

        // ra_0 depends on (k0, j); ra_1 on (k1, j). Broadcast each over the other chunk.
    ...
    pub fn new_verifier(params: BytecodeReadRafParams<F>) -> Self {
...
impl<F: Field> SumcheckInstance<F> for BytecodeReadRaf<F> {
    ...
    fn cache_openings(&self, accumulator: &mut Openings<F>, challenges: &[F]) {
        ...
        accumulator.append_dense(
            CommittedPolynomial::BytecodeRa(1),
            SumcheckId::BytecodeReadRaf,
            chunk_point(&r_k1),
    ...
    fn expected_output_claim(
        ...
        let (_, ra0) = accumulator.get_committed_polynomial_opening(
            CommittedPolynomial::BytecodeRa(0),
            SumcheckId::BytecodeReadRaf,
        );
        let (_, ra1) = accumulator.get_committed_polynomial_opening(
            CommittedPolynomial::BytecodeRa(1),
            SumcheckId::BytecodeReadRaf,
        );
...
mod tests {
    ...
    // Distinct rv keys for the stages (any existing virtual variants serve as opaque keys).
    fn rv_keys() -> Vec<(VirtualPolynomial, SumcheckId)> {
        vec![
    ...
    fn round_trip(seed: u64, log_k0: usize, log_k1: usize, log_t: usize, num_stages: usize) {
        ...
        let stages: Vec<ReadRafStage<F>> = (0..num_stages)
            .map(|s| ReadRafStage {
                r_cycle: rand_vec(&mut rng, log_t),
                val_addr: rand_vec(&mut rng, k),
                rv_key: keys[s],
            })
            .collect();

        // Per-stage rv_s = Σ_{j,k} ra(k,j)·eq_s(j)·Val_s(k) (γ-independent), so for any γ:
        // input_claim = Σ_s γ^s·rv_s = Σ_{j,k} ra0·ra1·Σ_s γ^s·eq_s·Val_s.
        ...
        let mut prover_t = Blake2bTranscript::<F>::new(b"bytecode-read-raf");
        let params =
            BytecodeReadRafParams::new([log_k0, log_k1], log_t, stages.clone(), &mut prover_t);
        let input_claim = params.input_claim(&prover_acc);
        let mut prover = BytecodeReadRaf::new_prover(params, [ra0.clone(), ra1.clone()]);
        ...
        let mut verifier_t = Blake2bTranscript::<F>::new(b"bytecode-read-raf");
        let vparams = BytecodeReadRafParams::new([log_k0, log_k1], log_t, stages, &mut verifier_t);
        let verifier = BytecodeReadRaf::new_verifier(vparams);
        ...
        let EvaluationClaim { point, value } =
            verify(&claim, &proof, &mut verifier_t).expect("bytecode read-raf must verify");
        assert_eq!(
            point, challenges,
    ...
    fn bytecode_read_raf_round_trip() {
        round_trip(0xB100, 1, 1, 2, 3);
        round_trip(0xB101, 2, 2, 3, 3);
        round_trip(0xB102, 1, 2, 4, 2);
        round_trip(0xB103, 2, 1, 3, 1);
    }

    #[test]
    fn tampered_proof_rejected() {
        let (log_k0, log_k1, log_t) = (2, 2, 3);
        let mut rng = Rng(0xB1FE);
        let k0 = 1usize << log_k0;
        let k1 = 1usize << log_k1;
        let t = 1usize << log_t;
        let ra0 = rand_vec(&mut rng, k0 * t);
        ...
        let params = BytecodeReadRafParams::new([log_k0, log_k1], log_t, stages, &mut prover_t);
        let input_claim = params.input_claim(&acc);
        let mut prover = BytecodeReadRaf::new_prover(params, [ra0, ra1]);
