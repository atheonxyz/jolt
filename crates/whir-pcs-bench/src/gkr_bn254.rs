...
use ark_ff::{One, Zero};
use jolt_field::{Field as JoltField, FieldAccumulator, Fr as JoltFr};
...
fn prove_single_instance_sumcheck(
    ...
    for k in 0..log_size {
        ...
        // SAFETY: capacity >= half (= 2^k ≤ max_half); every slot written exactly once.
        unsafe {
            vals.set_len(half);
        }
        vals.par_iter_mut().enumerate().for_each(|(i, dst)| {
        ...
        // SAFETY: capacity >= eq_unbound_init_len (= 2^{k-1} ≤ max_half).
        unsafe {
            eq_unbound.set_len(eq_unbound_init_len);
        }
        if k <= 1 {
            eq_unbound[0] = JoltFr::one();
        } else {
        ...
        for _round_idx in 0..k {
            ...
            let h0_j: JoltFr = round_poly.evaluate(JoltFr::zero());
            ...
            // SAFETY: capacity >= cur_half; every slot written exactly once.
            unsafe {
                vals_swap.set_len(cur_half);
            }
            vals_swap.par_iter_mut().enumerate().for_each(|(i, dst)| {
                let v0 = vals[2 * i];
                let v1 = vals[2 * i + 1];
                *dst = PairVals {
            ...
            if eq_unbound.len() > 1 {
                ...
                // SAFETY: capacity >= new_eq_len; every slot written exactly once.
                unsafe {
                ...
                eq_unbound_swap
                    ...
                    .enumerate()
                    .for_each(|(i, dst)| {
                        *dst = eq_unbound[2 * i] + eq_unbound[2 * i + 1];
                    });
...
fn compute_pair_q_values(
    ...
    type Acc = <JoltFr as JoltField>::Accumulator;
