...
use whir::algebra::fields::{Field256, Field64_3};
use whir::hash;
...
enum FieldChoice {
    /// BN254 scalar field (254-bit prime, Field256 — matches Dory)
    Bn254,
    /// Goldilocks cubic extension Fp3 (192-bit, soundness-correct at 128-bit security)
    GoldilocksFp3,
}
...
struct Args {
    ...
    /// Which scalar field WHIR commits to.
    #[arg(long, value_enum, default_value_t = FieldChoice::Bn254)]
    field: FieldChoice,
    ...
    /// Force the BN254 path to use the generic `gkr::run_gkr` (the hand-coded
    /// loop) instead of the Jolt-primitive-optimized `gkr_bn254::run_gkr`.
    /// A/B comparison against the same binary; no effect on the Goldilocks Fp3 path.
    #[arg(long, default_value_t = false)]
    bn254_generic_gkr: bool,
}
...
fn main() {
    ...
    let stats = match args.field {
        FieldChoice::Bn254 => {
            check_round_trip_bn254();
            let gkr_fn: RunGkrFn<Field256> = if args.bn254_generic_gkr {
                println!("[whir] BN254 GKR backend: generic (gkr::run_gkr)");
                gkr::run_gkr::<Field256>
            } else {
        ...
        FieldChoice::GoldilocksFp3 => {
            println!("[whir] field: Goldilocks cubic extension Fp3 (Field64_3)");
            run_bench::<Field64_3>(
                &args,
                &raw,
                "Goldilocks Fp3 (Field64_3)",
                gkr::run_gkr::<Field64_3>,
            )
        }
    };
    ...
    let gkr_summary: Option<serde_json::Value> = stats.gkr.as_ref().map(|g| {
        ...
        println!(
            "[logup*] combined commit + logup* median = {:.1}ms ({} runs)",
            median + gmedian,
            g.runs_ms.len(),
        );
