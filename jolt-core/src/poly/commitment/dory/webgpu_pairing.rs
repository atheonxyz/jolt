...
mod js_bridge {
    ...
    extern "C" {
        ...
        #[wasm_bindgen(js_namespace = ["globalThis"], js_name = "__jolt_gpu_multi_pairing")]
        pub async fn js_gpu_multi_pairing(
            g1_flat: &[u32],
            g2_flat: &[u32],
            num_pairs: u32,
        ) -> JsValue;
...
#[cfg(target_arch = "wasm32")]
pub async fn gpu_batch_multi_pairing(g1_groups: &[Vec<ArkG1>], g2_bases: &[ArkG2]) -> Vec<ArkGT> {
    use js_sys::Uint32Array;
    use rayon::prelude::*;
    use wasm_bindgen::JsCast;
    ...
    for g1_points in g1_groups {
        let n = g1_points.len();
        let gpu_count = if n < MIN_HYBRID_PAIRS {
            n
        } else {
            let gc = ((n as f64) * GPU_RATIO).round() as usize;
            gc.max(1).min(n - 1)
        };
        gpu_counts.push(gpu_count);
        group_offsets.push(total_gpu_pairs as u32);
        group_sizes.push(gpu_count as u32);
        max_gpu_count = max_gpu_count.max(gpu_count);
        total_gpu_pairs += gpu_count;
    }
    ...
    let per_group_g1: Vec<Vec<u32>> = g1_groups
        .par_iter()
        .zip(gpu_counts.par_iter())
        .map(|(g1_points, &gpu_count)| serialize_g1_points(&g1_points[..gpu_count]))
        .collect();

    let mut g1_flat = Vec::with_capacity(total_gpu_pairs * 2 * NUM_LIMBS);
    let mut g2_flat = Vec::with_capacity(total_gpu_pairs * 4 * NUM_LIMBS);
    for (g, &gpu_count) in gpu_counts.iter().enumerate() {
        g1_flat.extend_from_slice(&per_group_g1[g]);
        let g2_words = gpu_count * 4 * NUM_LIMBS;
        g2_flat.extend_from_slice(&g2_max_serialized[..g2_words]);
    }

    let group_sizes_u32: Vec<u32> = group_sizes;
    let group_offsets_u32: Vec<u32> = group_offsets;

    let gpu_future =
        js_bridge::js_gpu_batch_pairing(&g1_flat, &g2_flat, &group_sizes_u32, &group_offsets_u32);

    let cpu_millers: Vec<Option<Fq12>> = g1_groups
        .par_iter()
        .zip(gpu_counts.par_iter())
        .map(|(g1_points, &gpu_count)| {
            let n = g1_points.len();
            if gpu_count >= n {
                None
            } else {
                let g1_cpu = &g1_points[gpu_count..];
                let g2_cpu = &g2_bases[gpu_count..n];
                let miller = Bn254::multi_miller_loop(
                    g1_cpu.iter().map(|p| p.0.into_affine()),
                    g2_cpu.iter().map(|p| p.0.into_affine()),
                );
                Some(miller.0)
            }
        })
    ...
    let result_js = gpu_future.await;
    let result_u32_array: Uint32Array =
        result_js.dyn_into().expect("Expected Uint32Array from GPU");
    let result_words: Vec<u32> = result_u32_array.to_vec();

    let out: Vec<ArkGT> = (0..num_groups)
        .into_par_iter()
        .map(|g| {
            let start = g * FP12_WORDS;
            let end = start + FP12_WORDS;
            ...
            let combined_miller = match &cpu_millers[g] {
                Some(cpu_fp12) => gpu_miller * cpu_fp12,
                None => gpu_miller,
            };
...
#[cfg(target_arch = "wasm32")]
pub async fn gpu_multi_pair_g2_setup(g1_points: &[ArkG1], g2_bases: &[ArkG2]) -> ArkGT {
    let groups = vec![g1_points.to_vec()];
    let results = gpu_batch_multi_pairing(&groups, g2_bases).await;
    results.into_iter().next().unwrap()
}

#[cfg(target_arch = "wasm32")]
pub async fn gpu_flat_multi_pairing(g1_points: &[ArkG1], g2_bases: &[ArkG2]) -> ArkGT {
    use js_sys::Uint32Array;
    use wasm_bindgen::JsCast;
    ...
    let n = g1_points.len();
    assert_eq!(n, g2_bases.len(), "G1 and G2 lengths must match");

    if n == 0 {
        return ArkGT(Fq12::new(
            Fq6::new(
    ...
    let (filtered_g1, filtered_g2): (Vec<&ArkG1>, Vec<&ArkG2>) = g1_points
        .iter()
        .zip(g2_bases.iter())
        .filter(|(g1, _)| !g1.0.is_zero())
        .unzip();

    let n = filtered_g1.len();
    if n == 0 {
        return ArkGT(Fq12::new(
            Fq6::new(
                Fq2::new(Fq::from(1u64), Fq::from(0u64)),
    ...
    let g1_refs: Vec<ArkG1> = filtered_g1[..gpu_count].iter().map(|p| **p).collect();
    let g2_refs: Vec<ArkG2> = filtered_g2[..gpu_count].iter().map(|p| **p).collect();
    let g1_flat = serialize_g1_points(&g1_refs);
    let g2_flat = serialize_g2_points(&g2_refs);

    let gpu_future = js_bridge::js_gpu_multi_pairing(&g1_flat, &g2_flat, gpu_count as u32);

    let cpu_miller = if gpu_count < n {
        let g1_cpu: Vec<_> = filtered_g1[gpu_count..]
            .iter()
            .map(|p| p.0.into_affine())
            .collect();
        let g2_cpu: Vec<_> = filtered_g2[gpu_count..]
            .iter()
            .map(|p| p.0.into_affine())
            .collect();
        Some(Bn254::multi_miller_loop(g1_cpu.into_iter(), g2_cpu.into_iter()).0)
    } else {
        None
    };
    ...
    let result_u32_array: Uint32Array = result_js
        .dyn_into()
        .expect("Expected Uint32Array from GPU flat pairing");
    let result_words: Vec<u32> = result_u32_array.to_vec();
    let gpu_miller = deserialize_fq12(&result_words[..FP12_WORDS]);

    let combined_miller = match cpu_miller {
        Some(cpu_fp12) => gpu_miller * cpu_fp12,
        None => gpu_miller,
    };

    let pairing_result = Bn254::final_exponentiation(MillerLoopOutput::<Bn254>(combined_miller))
        .expect("Final exponentiation failed");
    ArkGT(pairing_result.0)
}
