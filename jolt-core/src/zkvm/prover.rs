    ...
    > JoltCpuProver<'a, F, C, PCS, ProofTranscript>
{
        ...
        >,
    {
        ...
        let (poly_gather_data, poly_metadata): (
            Vec<(Vec<u32>, Vec<u32>, u32, usize, usize)>,
                    ...
                    .map(|i| {
                        ...
                        match poly {
                            CommittedPolynomial::InstructionRa(d) => {
                                Some(self.one_hot_params.lookup_index_chunk(
                                    LookupQuery::<XLEN>::to_lookup_index(cycle),
                                    d,
                                ) as u8)
                            }
                            CommittedPolynomial::BytecodeRa(d) => {
                                Some(self.one_hot_params.bytecode_pc_chunk(
                            ...
                            .map(|addr| self.one_hot_params.ram_address_chunk(addr, d) as u8),
                            _ => None,
                        }
                    })
                    .collect();
        ...
        // Pre-serialize G2 bases once for both pairing dispatches (avoids redundant
        // projective→affine conversion + bigint serialization on each dispatch).
        let use_gpu_pairing = webgpu_msm::is_gpu_msm_available();
        let g2_serialized_cache: Option<Vec<u32>> = if use_gpu_pairing {
            let _g2_span = tracing::trace_span!("gpu_commit.serialize_g2_cache").entered();
            Some(webgpu_pairing::serialize_g2_points(
                &self.preprocessing.generators.g2_vec,
            ))
        } else {
            None
        };
        ...
        let mut onehot_gpu_buffer_size = 0usize;
        let mut onehot_poly_layout: Vec<(usize, usize)> = Vec::new();
        if let Some(batch_handle) = gpu_onehot_batch_handle {
            ...
            onehot_gpu_buffer_size = batch_result.gpu_buffer_size;
            onehot_poly_layout = batch_result.poly_layout;
        ...
        let onehot_gpu_pairing_handle =
            if use_gpu_pairing && !onehot_gpu_row_comms.is_empty() && onehot_gpu_buffer.is_some() {
                let _dispatch_pair_span =
                    tracing::trace_span!("gpu_commit.dispatch_pairing.onehot_buffer").entered();
                let g1_refs: Vec<&[ArkG1]> = onehot_gpu_row_comms
                    .iter()
                    .map(|(_, rc)| rc.as_slice())
                    .collect();
                let max_rows = g1_refs.iter().map(|g| g.len()).max().unwrap_or(0);
                let g2_bases: &[ArkG2] = &self.preprocessing.generators.g2_vec[..max_rows];
                Some(webgpu_pairing::dispatch_gpu_batch_pairing_from_buffer(
                    onehot_gpu_buffer
                        .as_ref()
                        .expect("onehot gpu buffer must exist when dispatching pairing"),
                    onehot_gpu_buffer_size,
                    &onehot_poly_layout,
                    g2_bases,
                    &g1_refs,
                    g2_serialized_cache.as_deref(),
        ...
        // NOTE: The retained GPU onehot buffer stores points in chunk-major order,
        // but combine_hints expects row-major order (transposed via row_idx = c + ki * rows_per_k).
        // Until a GPU scatter/transpose kernel is added, we skip the retained buffer path
        ...
        >,
    {
        ...
        let (joint_poly, poly_ids, hints, coeffs) = state.build_streaming_rlc_poly_only::<PCS>(
            self.one_hot_params.clone(),
            TraceSource::Materialized(Arc::clone(&self.trace)),
            streaming_data,
            opening_proof_hints,
        ...
        // GPU path: dispatch combine_hints to GPU, overlap CPU work (v_vec) while GPU runs.
        #[cfg(feature = "webgpu-pairing")]
        let (proof, _y_blinding) = {
            use crate::poly::commitment::dory::{
                webgpu_g2, webgpu_pairing, DoryCommitmentScheme, DoryOpeningProofHint,
            };
            ...
            if webgpu_g2::is_gpu_g2_available() && webgpu_pairing::is_gpu_combine_hints_available()
            {
                let _span = tracing::info_span!("gpu_g2_stage8").entered();

                let hint_rows: Vec<Vec<crate::poly::commitment::dory::ArkG1>> = dory_hints
                    .into_iter()
                    .map(DoryOpeningProofHint::into_rows)
                    .collect();
                let combine_handle =
                    webgpu_pairing::dispatch_gpu_combine_hints(&hint_rows, &ark_coeffs);

                // While GPU runs combine_hints, compute v_vec and prepare G2 data on CPU
                ...
                // Resolve combine_hints (GPU may already be done by now)
                let combined_rows = webgpu_pairing::resolve_gpu_combine_hints(combine_handle).await;
                let hint = DoryOpeningProofHint::from_rows(combined_rows);

                // Dispatch GPU g2 scalar mul non-blocking — prove_with_gpu_v2 will do
                // CPU work (affine cache, eval vectors, VMV message) while GPU runs,
                // then resolve the g2 result just before the reduce-and-fold loop.
                let g2_fin = setup.g2_vec[0].0;
                let g2_handle = webgpu_g2::dispatch_gpu_g2_scalar_mul(&g2_fin, &raw_scalars);
                ...
                let hint = DoryCommitmentScheme::combine_hints_gpu(dory_hints, &ark_coeffs).await;
                PCS::prove(
                    &self.preprocessing.generators,
                    &joint_poly,
                    &opening_point.r,
